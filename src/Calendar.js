...
class Month extends React.Component {
  render() {
    ...
    if (this.props.month === 1) {
      if (
        (this.props.year % 4 === 0 && this.props.year % 100 !== 0) ||
    ...
    return (
      <div className='month'>
        <h2 className='month__name'>
          <span
            className='month__name-previous'
            onClick={() => this.props.selectDate(previousMonth)}>
            {monthLabel[previousMonth % 12] || "December"}
          </span>
          <span className='month__name-current'>
            {monthLabel[this.props.month]}
          </span>
          <span
            className='month__name-next'
            onClick={() => this.props.selectDate(nextMonth)}>
            {monthLabel[nextMonth % 12]}
          </span>
        </h2>
        ...
        <div className='month__days'>
          {emptyDays.map((emptyDay) => (
            <div className='day' key={emptyDay}>
              <span></span>
            </div>
          ))}
          {monthDays.map((day) => {
            if (this.props.day === day) {
              return (
                <div key={day} className='day selected'>
                  <span>{day}</span>
                </div>
              );
            } else {
              return (
                <div key={day} className='day'>
                  <span
                    onClick={() =>
                      this.props.selectDate(this.props.month, day)
                    }>
                    {day}
                  </span>
                </div>
